...
import {
  ...
  sessionShowCommand,
  hookRegisterCommand,
  hookUnregisterCommand,
  cursorStopHookCommand,
} from './commands';
    ...
    switch (subcommand) {
      ...
      case 'config':
        ...
        if (configAction === 'show') {
          ...
          await configSetFeishuCommand(argv);
        } else {
      ...
      case '--heartbeat':
        await heartbeatCommand(argv);
        break;

      case 'session':
        const sessionAction = argv[3];
        if (sessionAction === 'show') {
          await sessionShowCommand(argv);
        } else if (sessionAction === 'close') {
          await sessionCloseCommand(argv);
        } else {
      ...
      case 'hook':
        ...
        if (hookAction === 'register') {
          await hookRegisterCommand(argv);
        } else if (hookAction === 'unregister') {
          await hookUnregisterCommand(argv);
        } else {
      ...
      case 'cursor-stop-hook':
        await cursorStopHookCommand(argv);
        break;

      default:
        // 默认行为：发送消息并等待回复
        await notifyCommand(argv);
        break;
    }
  } catch (error) {
