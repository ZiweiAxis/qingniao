...
export function ensureStopHook(root: string): void {
  ...
  const hasOurs = obj.hooks.stop.some(
    (e: { command?: string }) => e.command && (e.command.includes("cursor-stop-hook") || e.command.includes("skill-message-bridge") || e.command.includes("qingniao") || e.command.includes("@zwa/qingniao"))
  );
  if (!hasOurs) {
...
export function unregisterStopHook(root: string): void {
  ...
  try {
    ...
    obj.hooks.stop = obj.hooks.stop.filter(
      (e) => !e.command || (!e.command.includes("cursor-stop-hook") && !e.command.includes("skill-message-bridge") && !e.command.includes("qingniao") && !e.command.includes("@zwa/qingniao"))
    );
    fs.writeFileSync(hooksPath, JSON.stringify(obj, null, 2), "utf8");
  } catch {
    // ignore
