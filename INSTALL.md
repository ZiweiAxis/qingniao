...
命令行仍可直接用 npx，无需全局安装。**发布状态**：包名 `skill-message-bridge`，已上架 npm；亦可 `npm install github:ZiweiAxis/qingniao`。
...
| 方式 | 环境        | 安装方式 | 说明 |
|------|-------------|----------|------|
| **npm** | 任意 Node 项目 | `npm install skill-message-bridge` | 代码中 `require("skill-message-bridge")`；命令行 `npx skill-message-bridge "..."`；未发布前可用 `npm install github:ZiweiAxis/qingniao` |
